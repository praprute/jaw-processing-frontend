...
import { createReceiveFishyTask, getCustomerByBillTask } from '../../share-module/FishWeightBill/task'
...
const CreateFishyBillPage: NextPageWithLayout = () => {
    const [form] = Form.useForm()
    const navigation = useNavigation()
    const [dateBill, setDateBill] = useState(null)
    const createReceiveFishy = createReceiveFishyTask.useTask()
    const getCustomerByBill = getCustomerByBillTask.useTask()

    useEffect(() => {
        ;(async () => {
            await getCustomerByBill.onRequest({ type_bill: 6 })
        })()
    }, [])
    ...
    const handleSubmit = async (values: any) => {
        try {
            ...
            const res = await createReceiveFishy.onRequest(payload)
...
CreateFishyBillPage.getLayout = function getLayout(page: ReactElement) {
    return (
...
export default CreateFishyBillPage
